...
def main():
    ...
    if TEST_MODE:
        test_file = 'header_table/monthly/header_table_BETA_USW00094057_1.psv'
        test_file = 'header_table/monthly/header_table_BETA_USW00024139_1.psv'
        file_list = [test_file]
        file_idx = [0]
...
#        if len(header_table['source_id'].unique()) > 
        ...
        # Do a check so that symmetric difference happens
        ...
        mt = master_table
        print("pd.np.nan in list(mt['longitude'].unique()) or pd.np.nan in list(mt['latitude'].unique())")
        import pdb; pdb.set_trace()

        if pd.np.nan in list(mt['longitude'].unique()) or pd.np.nan in list(mt['latitude'].unique()):
            raise Exception('NANs in master table "longitude" or "latitude"!')
...
iset(ht['station_record_number']).intersection(set(scs['station_record_number']))

            sdkfljsdkfdsjflksdjf
           continue
...
#            'source_id_station': 'source_id',
        ...
        observation_columns = [
            ...
            'adjustment_id','traceability','advanced_qc','advanced_homogenisation','source_id','report_type','station_type',
            'location'
        ]
